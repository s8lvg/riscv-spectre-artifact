...
/*
 * Branchless JIT dispatch for Spectre-BTB mitigation (RISC-V only)
 * Instead of indirect jumps, we JIT a direct JAL instruction at runtime.
 */
#if defined(__riscv) && defined(UBPF_JIT_DISPATCH)

static inline uint32_t encode_jal(int32_t offset, int rd) {
    /* Encode RISC-V JAL instruction: jal rd, offset */
    return 0x6F
         | ((rd & 0x1F) << 7)
         | (((offset >> 12) & 0xFF) << 12)
         | (((offset >> 11) & 0x1) << 20)
...
/*
 * JIT slot in .text section. First word is overwritten with JAL at runtime.
 */
__attribute__((naked, used, section(".text.jit"), aligned(4)))
static void __jit_dispatch_region(void) {
    __asm__ volatile(
        ".balign 4\n"
        ".global ubpf_jit_slot\n"
        "ubpf_jit_slot:\n"
        ".word 0x00000013\n"  /* NOP - will be overwritten with JAL */
        ".word 0x00000013\n"  /* NOP */
    );
}

extern uint32_t ubpf_jit_slot[];

static int jit_dispatch_initialized = 0;

static int jit_dispatch_init(void) {
    if (jit_dispatch_initialized) return 0;

    /* Make the JIT slot page writable */
    uintptr_t page = (uintptr_t)ubpf_jit_slot & ~0xFFFUL;
    if (mprotect((void*)page, 4096, PROT_READ | PROT_WRITE | PROT_EXEC) != 0) {
        return -1;
    }
    jit_dispatch_initialized = 1;
    return 0;
}

/*
 * JIT dispatch: encode JAL to target, fence.i, then direct jump to jit slot.
 * The JAL (rd=0) at jit_slot jumps directly to the handler.
 * Handler then does `goto next_insn` which continues the interpreter loop.
 *
 * All branches are DIRECT - no indirect jumps:
 *   1. `j ubpf_jit_slot` - direct jump to known symbol
 *   2. JAL in jit_slot - direct jump to handler (encoded at runtime)
 *   3. `goto next_insn` - direct jump (compiler-known label)
 *
 * We use asm goto to tell the compiler that next_insn is reachable from here.
 */
#define JIT_DISPATCH(target, next_label) do { \
    intptr_t _offset = (char*)(target) - (char*)&ubpf_jit_slot[0]; \
    ubpf_jit_slot[0] = encode_jal((int32_t)_offset, 0); \
...
#endif /* __riscv && UBPF_JIT_DISPATCH */
    ...
    size_t stack_length)
{
    ...
    while (1) {
#if defined(__riscv) && defined(UBPF_JIT_DISPATCH)
        __asm__ volatile("" ::: "memory");  /* Prevent loop optimizations with JIT dispatch */
#endif
        ...
        /* Computed goto jump table for interpreter dispatch */
#ifdef __clang__
...
#pragma GCC diagnostic ignored "-Winitializer-overrides"
        ...
        static const void* jumptable[256] = {
            [0 ... 255] = &&DO_DEFAULT,
            [EBPF_OP_ADD_IMM] = &&DO_ADD_IMM,
            [EBPF_OP_ADD_REG] = &&DO_ADD_REG,
            [EBPF_OP_SUB_IMM] = &&DO_SUB_IMM,
            [EBPF_OP_SUB_REG] = &&DO_SUB_REG,
            [EBPF_OP_MUL_IMM] = &&DO_MUL_IMM,
            [EBPF_OP_MUL_REG] = &&DO_MUL_REG,
            [EBPF_OP_DIV_IMM] = &&DO_DIV_IMM,
            [EBPF_OP_DIV_REG] = &&DO_DIV_REG,
            [EBPF_OP_OR_IMM] = &&DO_OR_IMM,
            [EBPF_OP_OR_REG] = &&DO_OR_REG,
            [EBPF_OP_AND_IMM] = &&DO_AND_IMM,
            [EBPF_OP_AND_REG] = &&DO_AND_REG,
            [EBPF_OP_LSH_IMM] = &&DO_LSH_IMM,
            [EBPF_OP_LSH_REG] = &&DO_LSH_REG,
            [EBPF_OP_RSH_IMM] = &&DO_RSH_IMM,
            [EBPF_OP_RSH_REG] = &&DO_RSH_REG,
            [EBPF_OP_NEG] = &&DO_NEG,
            [EBPF_OP_MOD_IMM] = &&DO_MOD_IMM,
            [EBPF_OP_MOD_REG] = &&DO_MOD_REG,
            [EBPF_OP_XOR_IMM] = &&DO_XOR_IMM,
            [EBPF_OP_XOR_REG] = &&DO_XOR_REG,
            [EBPF_OP_MOV_IMM] = &&DO_MOV_IMM,
            [EBPF_OP_MOV_REG] = &&DO_MOV_REG,
            [EBPF_OP_ARSH_IMM] = &&DO_ARSH_IMM,
            [EBPF_OP_ARSH_REG] = &&DO_ARSH_REG,
            [EBPF_OP_LE] = &&DO_LE,
            [EBPF_OP_BE] = &&DO_BE,
            [EBPF_OP_ADD64_IMM] = &&DO_ADD64_IMM,
            [EBPF_OP_ADD64_REG] = &&DO_ADD64_REG,
            [EBPF_OP_SUB64_IMM] = &&DO_SUB64_IMM,
            [EBPF_OP_SUB64_REG] = &&DO_SUB64_REG,
            [EBPF_OP_MUL64_IMM] = &&DO_MUL64_IMM,
            [EBPF_OP_MUL64_REG] = &&DO_MUL64_REG,
            [EBPF_OP_DIV64_IMM] = &&DO_DIV64_IMM,
            [EBPF_OP_DIV64_REG] = &&DO_DIV64_REG,
            [EBPF_OP_OR64_IMM] = &&DO_OR64_IMM,
            [EBPF_OP_OR64_REG] = &&DO_OR64_REG,
            [EBPF_OP_AND64_IMM] = &&DO_AND64_IMM,
            [EBPF_OP_AND64_REG] = &&DO_AND64_REG,
            [EBPF_OP_LSH64_IMM] = &&DO_LSH64_IMM,
            [EBPF_OP_LSH64_REG] = &&DO_LSH64_REG,
            [EBPF_OP_RSH64_IMM] = &&DO_RSH64_IMM,
            [EBPF_OP_RSH64_REG] = &&DO_RSH64_REG,
            [EBPF_OP_NEG64] = &&DO_NEG64,
            [EBPF_OP_MOD64_IMM] = &&DO_MOD64_IMM,
            [EBPF_OP_MOD64_REG] = &&DO_MOD64_REG,
            [EBPF_OP_XOR64_IMM] = &&DO_XOR64_IMM,
            [EBPF_OP_XOR64_REG] = &&DO_XOR64_REG,
            [EBPF_OP_MOV64_IMM] = &&DO_MOV64_IMM,
            [EBPF_OP_MOV64_REG] = &&DO_MOV64_REG,
            [EBPF_OP_ARSH64_IMM] = &&DO_ARSH64_IMM,
            [EBPF_OP_ARSH64_REG] = &&DO_ARSH64_REG,
            [EBPF_OP_LDXW] = &&DO_LDXW,
            [EBPF_OP_LDXH] = &&DO_LDXH,
            [EBPF_OP_LDXB] = &&DO_LDXB,
            [EBPF_OP_LDXDW] = &&DO_LDXDW,
            [EBPF_OP_STW] = &&DO_STW,
            [EBPF_OP_STH] = &&DO_STH,
            [EBPF_OP_STB] = &&DO_STB,
            [EBPF_OP_STDW] = &&DO_STDW,
            [EBPF_OP_STXW] = &&DO_STXW,
            [EBPF_OP_STXH] = &&DO_STXH,
            [EBPF_OP_STXB] = &&DO_STXB,
            [EBPF_OP_STXDW] = &&DO_STXDW,
            [EBPF_OP_LDDW] = &&DO_LDDW,
            [EBPF_OP_JA] = &&DO_JA,
            [EBPF_OP_JEQ_IMM] = &&DO_JEQ_IMM,
            [EBPF_OP_JEQ_REG] = &&DO_JEQ_REG,
            [EBPF_OP_JEQ32_IMM] = &&DO_JEQ32_IMM,
            [EBPF_OP_JEQ32_REG] = &&DO_JEQ32_REG,
            [EBPF_OP_JGT_IMM] = &&DO_JGT_IMM,
            [EBPF_OP_JGT_REG] = &&DO_JGT_REG,
            [EBPF_OP_JGT32_IMM] = &&DO_JGT32_IMM,
            [EBPF_OP_JGT32_REG] = &&DO_JGT32_REG,
            [EBPF_OP_JGE_IMM] = &&DO_JGE_IMM,
            [EBPF_OP_JGE_REG] = &&DO_JGE_REG,
            [EBPF_OP_JGE32_IMM] = &&DO_JGE32_IMM,
            [EBPF_OP_JGE32_REG] = &&DO_JGE32_REG,
            [EBPF_OP_JLT_IMM] = &&DO_JLT_IMM,
            [EBPF_OP_JLT_REG] = &&DO_JLT_REG,
            [EBPF_OP_JLT32_IMM] = &&DO_JLT32_IMM,
            [EBPF_OP_JLT32_REG] = &&DO_JLT32_REG,
            [EBPF_OP_JLE_IMM] = &&DO_JLE_IMM,
            [EBPF_OP_JLE_REG] = &&DO_JLE_REG,
            [EBPF_OP_JLE32_IMM] = &&DO_JLE32_IMM,
            [EBPF_OP_JLE32_REG] = &&DO_JLE32_REG,
            [EBPF_OP_JSET_IMM] = &&DO_JSET_IMM,
            [EBPF_OP_JSET_REG] = &&DO_JSET_REG,
            [EBPF_OP_JSET32_IMM] = &&DO_JSET32_IMM,
            [EBPF_OP_JSET32_REG] = &&DO_JSET32_REG,
            [EBPF_OP_JNE_IMM] = &&DO_JNE_IMM,
            [EBPF_OP_JNE_REG] = &&DO_JNE_REG,
            [EBPF_OP_JNE32_IMM] = &&DO_JNE32_IMM,
            [EBPF_OP_JNE32_REG] = &&DO_JNE32_REG,
            [EBPF_OP_JSGT_IMM] = &&DO_JSGT_IMM,
            [EBPF_OP_JSGT_REG] = &&DO_JSGT_REG,
            [EBPF_OP_JSGT32_IMM] = &&DO_JSGT32_IMM,
            [EBPF_OP_JSGT32_REG] = &&DO_JSGT32_REG,
            [EBPF_OP_JSGE_IMM] = &&DO_JSGE_IMM,
            [EBPF_OP_JSGE_REG] = &&DO_JSGE_REG,
            [EBPF_OP_JSGE32_IMM] = &&DO_JSGE32_IMM,
            [EBPF_OP_JSGE32_REG] = &&DO_JSGE32_REG,
            [EBPF_OP_JSLT_IMM] = &&DO_JSLT_IMM,
            [EBPF_OP_JSLT_REG] = &&DO_JSLT_REG,
            [EBPF_OP_JSLT32_IMM] = &&DO_JSLT32_IMM,
            [EBPF_OP_JSLT32_REG] = &&DO_JSLT32_REG,
            [EBPF_OP_JSLE_IMM] = &&DO_JSLE_IMM,
            [EBPF_OP_JSLE_REG] = &&DO_JSLE_REG,
            [EBPF_OP_JSLE32_IMM] = &&DO_JSLE32_IMM,
            [EBPF_OP_JSLE32_REG] = &&DO_JSLE32_REG,
            [EBPF_OP_EXIT] = &&DO_EXIT,
            [EBPF_OP_CALL] = &&DO_CALL,
            [EBPF_OP_ATOMIC_STORE] = &&DO_ATOMIC_STORE,
            [EBPF_OP_ATOMIC32_STORE] = &&DO_ATOMIC32_STORE,
        };
#ifdef __clang__
...
#if defined(__riscv) && defined(UBPF_JIT_DISPATCH)
        /* Mitigated dispatch: JIT a direct JAL instruction.
         * Volatile fake goto forces compiler to preserve distinct label addresses. */
        { volatile int _v = 0; if (_v) goto *jumptable[inst.opcode]; }
        JIT_DISPATCH(jumptable[inst.opcode], next_insn);
#else
        /* Unmitigated dispatch: indirect jump via computed goto */
        goto *jumptable[inst.opcode];
#endif
